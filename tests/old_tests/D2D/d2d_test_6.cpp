...
//**************************************************//
  ...
  #pragma omp parallel num_threads(num_thr) private(cpu, node)
  {
    ...
    switch(cpu){
      case 0:
        ...
        x_arr_0 = (int *)omp_alloc(size_c*sizeof(int), x_alloc);
      break;
      case 20:
        if (v_flag) printf("Alloc Host_1: CPU core %.2u NUMA node %u Thread %.2d\n", cpu, node, omp_get_thread_num());
        x_arr_1 = (int *)omp_alloc(size_c*sizeof(int), y_alloc);
      break;
      // default:
  ...
  // Allocate device memory
  x_ptr[1] = omp_target_alloc(size_c, 4);
...
//**************************************************//
  ...
  #pragma omp parallel num_threads(num_thr) private(cpu, node)
  {
    ...
    switch(cpu){
      ...
      case 20:
        ...
        // x_arr_1 = (int *)omp_alloc(2*size_c, x_alloc);
      break;
      // default:
        // if (v_flag) printf("Default: CPU core %.2u NUMA node %u Thread %.2d\n", cpu, node, omp_get_thread_num());
    }
  ...
  x_ptr[1] = omp_target_alloc(size_c, 3);
  x_ptr[2] = omp_target_alloc(size_c, 4);
  ...
  omp_target_free(x_ptr[0], 0);
  omp_target_free(x_ptr[1], 3);
  omp_target_free(x_ptr[2], 4);
...
// PROG=d2d_test_6; clang++ -fopenmp -fopenmp-targets=nvptx64 -o $PROG.x --cuda-gpu-arch=sm_70 -L/soft/compilers/cuda/cuda-11.0.2/lib64 -L/soft/compilers/cuda/cuda-11.0.2/targets/x86_64-linux/lib/ -I/soft/compilers/cuda/cuda-11.0.2/include -ldl -lcudart -pthread $PROG.cpp

//**************************************************//
//                        RUN                       //
//**************************************************//

// OMP_PROC_BIND=spread taskset -c 0,1,20 ./d2d_test_6.x

// nsys profile -o d2d_test_6_6G_v2 ./run_one.sh

// nsys profile -o d2d_test_6_6G ./d2d_test_6.x
// taskset -c 0,1,20 ./d2d_test_6.x

// OMP_PROC_BIND=true OMP_DYNAMIC=true OMP_MAX_ACTIVE_LEVELS=2 OMP_DISPLAY_ENV=VERBOSE

// ./d2d_test_6.x 
// ./d2d_test_6.x>out_test.o  2>&1
// nsys profile -o test_prof --stats=true ./d2d_test_6.x
// nsys profile -o d2d_test_6_6G_0 ./d2d_test_6.x

// OMP_PROC_BIND=true nsys profile -o d2d_test_6_6G_1 ./d2d_test_6.x
// OMP_DYNAMIC=true nsys profile -o d2d_test_6_6G_2 ./d2d_test_6.x
// OMP_MAX_ACTIVE_LEVELS=2 nsys profile -o d2d_test_6_6G_3 ./d2d_test_6.x
// OMP_PROC_BIND=true OMP_DYNAMIC=true nsys profile -o d2d_test_6_6G_4 ./d2d_test_6.x
// OMP_PROC_BIND=true OMP_DYNAMIC=true OMP_MAX_ACTIVE_LEVELS=2 nsys profile -o d2d_test_6_6G_5 ./d2d_test_6.x
// OMP_PROC_BIND=spread,close OMP_DYNAMIC=true nsys profile -o d2d_test_6_6G_5 ./d2d_test_6.x
