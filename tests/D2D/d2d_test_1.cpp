...
//**************************************************//
  ...
  // Allocate device memory
  dest_no = 2;
  x_ptr[1] = omp_target_alloc(size/dest_no, 1);
  ...
  #pragma omp parallel shared(x_ptr)
  {
    #pragma omp single
    {
      for (int dev = 0; dev < dest_no; ++dev)
      {
        #pragma omp task depend(out:x_ptr[dev]) firstprivate(dev) shared(x_ptr)
        {
          // printf("To Main %d\n", main_arr[i]);
          omp_target_memcpy(
            x_ptr[dev],                         // dst
            ...
            size/dest_no,                       // length 
            0,                                  // dst_offset
            dev*size/dest_no,                   // src_offset, 
            dev,                                // dst_device_num
            omp_get_initial_device()            // src_device_num
          );
        }
      }
      #pragma omp task shared(x_ptr) depend(in:x_ptr[1]) shared(x_ptr)
      {
        // printf("To Main %d\n", main_arr[i]);
        omp_target_memcpy(
          x_ptr[0],                           // dst
...
//**************************************************//
  ...
  dest_no = 3;
  for (int dev = 1; dev < dest_no; ++dev)
    x_ptr[dev] = omp_target_alloc(size/dest_no, dev);
  ...
  #pragma omp parallel shared(x_ptr)
  {
    #pragma omp single
    {
      ...
      for (int dev = 1; dev < dest_no; ++dev)
      {
        #pragma omp task shared(x_ptr) depend(in:x_ptr[dev]) shared(x_ptr)
        {
          // printf("To Main %d\n", main_arr[i]);
          omp_target_memcpy(
            x_ptr[0],                           // dst
...
// nsys profile -o broadcast21_100M ./d2d_test_1.x
